...
class _MOSSv1d6ImageProcessor:
    """Minimal image_processor attribute for call_hf_processor_mm_only compat."""

    def __init__(self, adapter: "MOSSv1d6HFProcessorAdapter") -> None:
...
class MOSSv1d6HFProcessorAdapter:
    ...
    def __init__(self, info: "MOSSv1d6ProcessingInfo") -> None:
        self._info = info
        self._img_processor: ImageProcessPacking | None = None
        self.image_processor = _MOSSv1d6ImageProcessor(self)
...
class MOSSv1d6ProcessingInfo(BaseProcessingInfo):
    def get_hf_config(self):
        return self.ctx.get_hf_config(MOSSv1d6Config)
    ...
    def get_hf_processor(self, **kwargs: object) -> MOSSv1d6HFProcessorAdapter:
        del kwargs
        if getattr(self, "_moss_hf_processor", None) is None:
            self._moss_hf_processor = MOSSv1d6HFProcessorAdapter(self)
        return self._moss_hf_processor


class MOSSv1d6DummyInputsBuilder(BaseDummyInputsBuilder[MOSSv1d6ProcessingInfo]):
...
class MOSSv1d6MultiModalProcessor(BaseMultiModalProcessor[MOSSv1d6ProcessingInfo]):
    def _hf_processor_applies_updates(
        ...
        MOSS uses ``MOSSv1d6HFProcessorAdapter`` (encode text + tensors only).
...
class MOSSv1d6Rotary2DEmbedding(nn.Module):
...
class MOSSv1d6VisMLP(nn.Module):
...
class MOSSv1d6VisAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class MOSSv1d6VisTransformerBlock(nn.Module):
    def __init__(self, config: MOSSv1d6VisionConfig):
        super().__init__()
        self.config = config
        self.attention = MOSSv1d6VisAttention(config=config)
        self.attention_norm = RMSNorm(config.hidden_size, eps=config.eps)
        self.ffn_norm = RMSNorm(config.hidden_size, eps=config.eps)
        self.feed_forward = MOSSv1d6VisMLP(config)
...
class MOSSv1d6VisVisionTransformerBlocks(nn.Module):
    def __init__(self, config: MOSSv1d6VisionConfig):
        super().__init__()
        self.config = config
        self.layers = torch.nn.ModuleList()
        for _ in range(config.num_hidden_layers):
            self.layers.append(MOSSv1d6VisTransformerBlock(config=config))
...
class MOSSv1d6VisionTransformer(nn.Module):
    def __init__(
            self,
            config: MOSSv1d6VisionConfig
    ):
        super().__init__()
        self.config = config
        ...
        self.rope_pos_embedding = MOSSv1d6Rotary2DEmbedding(
            dim=config.head_dim,
        ...
        self.transformer = MOSSv1d6VisVisionTransformerBlocks(config)

        if self.config.model_path is not None:
            raise NotImplementedError

    @classmethod
    def _from_config(cls, config: MOSSv1d6VisionConfig) -> "MOSSv1d6VisionTransformer":
        if isinstance(config, dict):
            return cls.from_dict(config)
        return cls(config)

    @classmethod
    def from_dict(cls, config: dict) -> "MOSSv1d6VisionTransformer":
        vit_config = MOSSv1d6VisionConfig.from_dict(config)
...
class MOSSv1d6PatchMergerSwiGLU(nn.Module):
    """https://arxiv.org/pdf/2002.05202"""
...
class MOSSv1d6PatchMerger(nn.Module):
    def __init__(
        self,
        config: dict
    ) -> None:
        ...
        self.mlp = MOSSv1d6PatchMergerSwiGLU(
...
class MOSSv1d6Attention(nn.Module):
...
class MOSSv1d6DecoderLayer(nn.Module):
    def __init__(
        self,
        config: Any,
        cache_config: CacheConfig | None = None,
        ...
        self.self_attn = MOSSv1d6Attention(
...
def moss_v1d6_decoder_model_invariants(
...
@support_torch_compile(shape_invariants=moss_v1d6_decoder_model_invariants)
class MOSSv1d6DecoderModel(nn.Module):
    def __init__(
        self,
        *,
        ...
        self.start_layer, self.end_layer, self.layers = make_layers(
            ...
            lambda prefix: MOSSv1d6DecoderLayer(
                config=config,
                cache_config=cache_config,
                quant_config=quant_config,
                prefix=prefix,
...
class MOSSv1d6ForCausalLM(nn.Module, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        self.config = dec_cfg
        self.model = MOSSv1d6DecoderModel(
...
@MULTIMODAL_REGISTRY.register_processor(
    MOSSv1d6MultiModalProcessor,
    info=MOSSv1d6ProcessingInfo,
    dummy_inputs=MOSSv1d6DummyInputsBuilder,
)
class MOSSv1d6VLForConditionalGeneration(nn.Module, SupportsMultiModal, SupportsPP):
    """vLLM-compatible MOSS v1.6 VLM.

    encoder  →  compressor  →  language_model (MOSSv1d6ForCausalLM with paged attention)
    """
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        super().__init__()
        config: MOSSv1d6Config = vllm_config.model_config.hf_config
        self.config = config
        ...
        with self._mark_tower_model(vllm_config, "image"):
            self.encoder = MOSSv1d6VisionTransformer._from_config(config.encoder_config)
            self.compressor = MOSSv1d6PatchMerger(config=config.compressor_config)
            self.encoder_dtype = self.encoder.patch_conv.weight.dtype

        with self._mark_language_model(vllm_config):
            self.language_model = MOSSv1d6ForCausalLM(
                vllm_config=vllm_config,
